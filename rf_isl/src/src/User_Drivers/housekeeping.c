...
float get_power_meter_measurement(void) {

	float power;
	float rms_voltage;

	/* First, read the adc_value */
	rms_voltage = (float) (adc_buffer[HK_POWER_METER_POS] * get_ref_voltage() / HK_ADC_FULL_SCALE);
	rms_voltage *= 1000; /* From V to mV */
	/* These 41.97 dB has to be added because there is a line that generates this loss */
	power = (rms_voltage / PM_CALIB_SLOPE) + PM_CALIB_INTERCEPT + 41.97;
	return power;
}
