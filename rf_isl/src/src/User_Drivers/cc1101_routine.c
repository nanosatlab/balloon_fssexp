...
static uint8_t actual_rssi = 0;
static uint32_t packet_rx_count = 0;
static uint32_t packet_rx_error = 0;
static uint32_t packet_tx_count = 0;
static uint32_t packet_tx_error = 0;
static float	installed_freq = 0;
...
/*
static void     transmission_mode(void);
static void     reception_mode(void);
*/
...
void get_cc1101_statistics(cc1101_external_info_t *cc1101_info)
{
    ...
    cc1101_info->packet_rx_count =  packet_rx_count;
    cc1101_info->packet_errors_corrected = packet_rx_error;
    cc1101_info->packet_tx_count = packet_tx_count;
    cc1101_info->packet_not_tx_count = packet_tx_error;
    cc1101_info->actual_rssi = actual_rssi;
    //cc1101_info->transmitted_power = transmitted_power;
    cc1101_info->freq = installed_freq;
}
...
// ------------------------------------------------------------------------------------------------
{
    ...
    if(radio_int_data.mode == RADIOMODE_RX) {
        ...
        if(int_line) {
            ...
            xTaskNotify(CommsTxTaskHandle, COMMS_NOTIFY_END_TX, eSetBits);
            radio_turn_rx_isr(radio_int_data.spi_parms);
        }
    }
    return ret;
...
int init_radio_config(spi_parms_t * spi_parms, radio_parms_t * radio_parms)
{
    ...
    for(index = 0; index < sizeof(patable); index++) {
        //patable[index] = 0xC8; /* Transmitted power 0xC8 (7 dBm) - Best option */
        patable[index] = 0xC8;
    }
    ...
    //   2 (10): ±BW CHAN /4
    //   3 (11): ±BW CHAN /2
    CC_SPIWriteReg(spi_parms, CC11xx_FOCCFG,   0x16); // Freq Offset Compens. Config
    ...
    //   6 (110): 40 dB
    //   7 (111): 42 dB
//    CC_SPIWriteReg(spi_parms, CC11xx_AGCCTRL2, 0x46); // AGC control.
    CC_SPIWriteReg(spi_parms, CC11xx_AGCCTRL2, 0x07); /* 0x07 - Best option */ // AGC control.
    ...
    CC_SPIWriteReg(spi_parms, CC11xx_AGCCTRL1, 0x48); /* 0x48 - best option */ // AGC control.
    ...
    //   2 (10):       32: 12 dB
    //   3 (11):       64: 16 dB
    //CC_SPIWriteReg(spi_parms, CC11xx_AGCCTRL0, 0x91); // AGC control.
//    CC_SPIWriteReg(spi_parms, CC11xx_AGCCTRL0, 0xB3); // AGC control.
      CC_SPIWriteReg(spi_parms, CC11xx_AGCCTRL0, 0xC3); // AGC control.
...
void transmission_mode(void)
{
	/* Reception Mode is the default mode of the RF ISL Model
	     * The reception mode shall deactivate:
	     * - Low Noise Amplifier (LNA): LNA_EN_PIN = 1
	     * - Switch at Reception mode: SW_CONTROL_PIN = 0
	     * The reception mode shall activate:
	     * - Power Amplifier (PA): PAN_EN_PIN = 1
	     * - Power Meter (PM): PM_EN_PIN = 1
	     */
	    HAL_GPIO_WritePin(SW_CONTROL_GPIO_Port, SW_CONTROL_Pin, GPIO_PIN_SET);
	    HAL_GPIO_WritePin(LNA_EN_GPIO_Port, LNA_EN_Pin, GPIO_PIN_SET);
	    HAL_GPIO_WritePin(PA_EN_GPIO_Port, PA_EN_Pin, GPIO_PIN_SET);
	    HAL_GPIO_WritePin(PM_EN_GPIO_Port, PM_EN_Pin, GPIO_PIN_SET);
}

void reception_mode(void)
{
	/* Reception Mode is the default mode of the RF ISL Model
	 * The reception mode shall activate:
	 * - Low Noise Amplifier (LNA): LNA_EN_PIN = 0
	 * - Switch at Reception mode: SW_CONTROL_PIN = 1
	 * The reception mode shall deactivate:
	 * - Power Amplifier (PA): PAN_EN_PIN = 0
	 * - Power Meter (PM): PM_EN_PIN = 0
	 */
	HAL_GPIO_WritePin(SW_CONTROL_GPIO_Port, SW_CONTROL_Pin, GPIO_PIN_RESET);
	HAL_GPIO_WritePin(LNA_EN_GPIO_Port, LNA_EN_Pin, GPIO_PIN_RESET);
	HAL_GPIO_WritePin(PA_EN_GPIO_Port, PA_EN_Pin, GPIO_PIN_RESET);
	HAL_GPIO_WritePin(PM_EN_GPIO_Port, PM_EN_Pin, GPIO_PIN_RESET);
}
...
void radio_turn_tx(spi_parms_t *spi_parms)
{
    ...
    do{
    	/* Send the command (i.e. CC11xx_STX) to the CC1101 Transceiver to transmit the data
    	 * that has already been stored in the TXFIFO.
    	 */
        CC_SPIStrobe(radio_int_data.spi_parms, CC11xx_STX);

        /* Read the state of the CC1101 Transceiver to verify if the transmission has been
         * done correctly.
         */
        CC_SPIReadStatus(radio_int_data.spi_parms, CC11xx_MARCSTATE, &state);

        if(state == CC11xx_STATE_TXFIFO_UNDERFLOW || state == CC11xx_STATE_RXFIFO_OVERFLOW) {
            /* The Transmission has finished, the CC1101 Transceiver shall go to the idle
             * state.
             */
        	radio_turn_idle(spi_parms);
        }
    ...
    /* Now the CC1101 is in Trasmission Mode (i.e. CC11xx_STATE_TX); Then, it is possible
     * to read the power; Notify the ControlTask to perform the corresponding measurement.
     */
    xTaskNotify(ControlTaskHandle, CTRL_HK_POWER_MEASURE, eSetBits);
}
...
// ------------------------------------------------------------------------------------------------
{
    ...
    /* If after that timeout the channel is still busy... something wrong happened, just mark as packet_not_tx, but send it anyways */
...
#endif
    /* Wake up GDO */
    xTaskNotify(CommsTaskHandle, COMMS_NOTIFY_SEND_REQ, eSetBits);
    return 0;
}
...
{
	TransceiverConfiguration trans_conf;
	/* Read EEPROM parameters */
	read_eeprom(EP_EEPROM_ID_TX_FREQ, trans_conf.raw, sizeof(TransceiverConfiguration));

	if(trans_conf.fields.freq < 500e6f && trans_conf.fields.freq > 400e6f) {
		installed_freq = trans_conf.fields.freq;
		set_freq_parameters(trans_conf.fields.freq, trans_conf.fields.freq, 150e3f, 0.0f, &radio_parms);
	} else {
		installed_freq = 437.350e6f;
		set_freq_parameters(437.350e6f, 437.350e6f, 150e3f, 0.0f, &radio_parms);
	}
	//set_sync_parameters(PREAMBLE_4, SYNC_30_over_32, 500, &radio_parms);
	set_sync_parameters(PREAMBLE_24, SYNC_30_over_32, 500, &radio_parms);
	//set_packet_parameters(false, true, &radio_parms);
    set_packet_parameters(true, true, &radio_parms);
...
void cc1101_work(void)
{
    ...
    /* Init done */
    while(1) {
        signals_to_wait = GDO_NOTIFY_GDO0 | GDO_NOTIFY_GDO2 | COMMS_NOTIFY_SEND_REQ | COMMS_NOTIFY_RESET;
        if(xTaskNotifyWait(0, signals_to_wait, &signal_received, COMMS_WAIT_TIME/portTICK_PERIOD_MS) == pdTRUE) {
            if(signal_received & GDO_NOTIFY_GDO0) {
                if(gdo0_isr() == func_data) {
                    if(set_new_packet_to_chunk(&hchunk_rx, &rx_radio_packet, rx_packet_buffer.fields.payload) > 0) {
                        ...
                        if(set_simple_link_packet(ll_packet, _ntohs(ll_packet->fields.len) + LINK_LAYER_HEADER_SIZE, 0, 0, &rx_packet_buffer) > 0) {
            ...
            if(signal_received & COMMS_NOTIFY_SEND_REQ) {
                ...
                initial_tx_count =
                        (radio_int_data.tx_count > CC11xx_FIFO_SIZE-1 ? CC11xx_FIFO_SIZE-1 : radio_int_data.tx_count);
                // Initial fill of TX FIFO
                CC_SPIWriteBurstReg(radio_int_data.spi_parms, CC11xx_TXFIFO,
                                        (uint8_t *) radio_int_data.tx_buf, initial_tx_count);
                radio_int_data.byte_index = initial_tx_count;
                radio_int_data.bytes_remaining = radio_int_data.tx_count - initial_tx_count;

                /* Start transmission of data using the CC1101 Transceiver */
                radio_turn_tx(radio_int_data.spi_parms);
            }
            if(signal_received & COMMS_NOTIFY_RESET) {
                init_radio = false;
                initialize_cc1101();
            }
        } else {
        ...
        xTaskNotify(ControlTaskHandle, CTRL_WDT_COMMS_RESET, eSetBits);
    }
}
...
void csma_tx_work(void)
{
    ...
    while(1) {
        ...
        if(xQueueReceive(RadioPacketTxQueueHandle, &tx_packet_buffer, 5000/portTICK_PERIOD_MS) == pdTRUE) {
            ll_packet = (link_layer_packet_t *) tx_packet_buffer.fields.payload;
            while(get_new_packet_from_chunk(&hchunk_tx, ll_packet->raw, _ntohs(ll_packet->fields.len) + LINK_LAYER_HEADER_SIZE,
                                                tx_packet_buffer.fields.config2, &tx_radio_packet) > 0) {
                if(radio_send_packet(&tx_radio_packet) == 0) {
                    if(xTaskNotifyWait( 0, COMMS_NOTIFY_END_TX, &signal_received,
                                            COMMS_WAIT_TIME/portTICK_PERIOD_MS) != pdTRUE) {
                        /* something went bad, reset the CC1101 */
                        xTaskNotify(CommsTaskHandle, COMMS_NOTIFY_RESET, eSetBits);
                    }
                }
        ...
        xTaskNotify(ControlTaskHandle, CTRL_WDT_COMMS_TX_RESET, eSetBits);
    }
}
