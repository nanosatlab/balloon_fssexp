...
static int process_command(simple_link_packet_t *packet)
{
    ...
    if(packet->fields.config1 == (uint8_t) data_frame) {
        ...
        if(xQueueSend(RadioPacketTxQueueHandle, packet, 0) != pdTRUE) {
            ...
            ret = set_simple_link_packet(NULL, 0, ack_frame, 0, packet);
            send_kiss_packet(0, packet, ret);
        }
    }else if(packet->fields.config1 == hk_frame) {
    	/* Retrieve Housekeeping */
        if(xQueueSend(ControlPacketQueueHandle, packet, 0) != pdTRUE) {
            ret = set_simple_link_packet(NULL, 0, nack_frame, 0, packet);
            send_kiss_packet(0, packet, ret);
        } else {
            xTaskNotify(ControlTaskHandle, CTRL_NEW_PACKET, eSetBits);
        }
    }else if(packet->fields.config1 == request_frame) {
    	/* Request a received packet */
    	...
    	/* Read from the command the value */
    	memset(&trans_conf, 0, sizeof(trans_conf));

    	//memcpy(&trans_conf.fields.freq, packet->fields.payload, packet->fields.len);
    	trans_conf.float_array[0] = packet->fields.payload[3];
    	trans_conf.float_array[1] = packet->fields.payload[2];
    	trans_conf.float_array[2] = packet->fields.payload[1];
    	trans_conf.float_array[3] = packet->fields.payload[0];

    	/* Store the value in the EEPROM */
    	write_eeprom(EP_EEPROM_ID_TX_FREQ, trans_conf.raw, sizeof(TransceiverConfiguration));
