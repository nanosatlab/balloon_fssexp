...
static SysBootCountEeprom sysbootcount;
static float transmitted_power = 75.0f;
...
void GetModuleHKData(comms_hk_data_t *data)
{
    ...
    data->control.tx_remaining = uxQueueMessagesWaiting(RadioPacketTxQueueHandle);
    ...
    data->housekeeping.transmit_power = (uint8_t) transmitted_power;

    data->housekeeping.boot_count = sysbootcount.fields.system_boot_count;
    data->housekeeping.freq = cc1101_info.freq;
}
  ...
  /* USER CODE BEGIN 5 */
    ...
    for(;;) {
        ...
        for(;;) {
            ...
            wdt_time = xTaskGetTickCount();

        	signals_to_wait = CTRL_WDT_COMMS_TX_RESET | CTRL_WDT_COMMS_RESET |
        						CTRL_WDT_INTERFACE_RESET | CTRL_NEW_PACKET | CTRL_HK_POWER_MEASURE;
        	xTaskNotifyWait(0, signals_to_wait, &signal_received, 2000);
        	            /* here we shall... */

        	wdt_time = xTaskGetTickCount() - wdt_time;

        	if((signal_received & CTRL_NEW_PACKET)) {
                if(xQueueReceive(ControlPacketQueueHandle, &control_packet, 0) == pdTRUE) {
                    /* Control packet asks for GetModuleHKData */
                    GetModuleHKData(&comms_data);
                    ret = set_simple_link_packet(&comms_data, sizeof(comms_data), hk_frame, 0, &control_packet);
                    send_kiss_packet(0, &control_packet, ret);
                }
        	} else if(wdt_time >= 2000) {
				GetModuleHKData(&comms_data);
			}
            ...
            if((signal_received & CTRL_WDT_INTERFACE_RESET)) {
                ...
                wdt_interface += wdt_time;
            }
            if((signal_received & CTRL_HK_POWER_MEASURE)) {
            	/* First, refresh the values of the ADCs */
            	refresh_housekeeping();
            	/* Then read the output power and store it in a variable */
            	transmitted_power = get_power_meter_measurement();
            }

            /* if the tasks have not refreshed WDT in 10 seconds, kill them! */
            if(wdt_interface > 10000 || wdt_comms > 10000 || wdt_comms_tx > 10000) {
